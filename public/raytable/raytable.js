...
; (function (factory) {
    ...
    TableClass.utils = {
        ...
        loading: function (type) {
            ...
            var jqobjs = tableCache.jqobjs[that.tableIndex],
                $tbox = jqobjs.tb_box,
                $loading = jqobjs.tb_box.find("div.raytable-loading");
        ...
        renderData: function () {
            ...
            $(data).each(function (a, b) {
                ...
                if (options.expandRow) {
                    trs.push('<td><a class="raytable-expand"></a></td>');
                }
                ...
                $.each(cols, function (c, opt) {
                    ...
                    if (!options.nowrap)
                        td.push(' raytable-cell-data-' + that.tableIndex + '-' + field);
                    ...
                    td.push(options.nowrap ? '<div class="' + cellCss + ' raytable-cell-data-' + that.tableIndex + '-' + field + '">' + value + '</div>' : value);
                    td.push('</td>');
            ...
            if (trs_fixed_l.length > 0) {
                if ($fixedtbodyl.length === 0) {
                    $('<div class="raytable-body-noscroll" />').html($('<div class="raytable-body-box" />')
                        .html('<table class="raytable-body"' + (options.even ? ' ray-even' : '') + '>' + trs_fixed_l.join('') + '</table>'))
            ...
            if (trs_fixed_r.length > 0) {
                if ($fixedtbodyr.length === 0) {
                    $('<div class="raytable-body-noscroll" />').html($('<div class="raytable-body-box" />')
                        .html('<table class="raytable-body"' + (options.even ? ' ray-even' : '') + '>' + trs_fixed_r.join('') + '</table>'))
                        .appendTo(jqobjs.tb_fixed_r);
                    //添加补丁
                    $('<div class="raytable-fixed-amend"/>').appendTo(jqobjs.tb_box);
                } else {
        ...
        fullHeight: function () {
            ...
            var lastTrs = jqobjs.tb_box.find(".raytable-body").find("tr:last");
        ...
        fullWidth: function () {
            ...
            var clientH = Math.min(tbodydiv.clientHeight, jqobjs.tb_body.height()),//取bodydiv和table的最小高度
                $tbody_fixed_box_noscroll = jqobjs.tb_box.find("div.raytable-fixed .raytable-body-noscroll");
            ...
            if ($tbodydiv.find(".raytable-body").outerWidth() < tbodydiv.clientWidth) {
        ...
        modifyFixedHeight: function (isneedAmendHead) {
            ...
            if (!options.nowrap) {
                ...
                var $tbody = jqobjs.tb_body.find(">tbody"),
                    body_fixed = jqobjs.tb_fixed.find(".raytable-body-box>table");
            ...
            if (isneedAmendHead || options.autoColNums > 0) {
                ...
                //注意多行固定问题，倒着修改正好
                var head_fixed_l = jqobjs.tb_fixed_l.find(".raytable-head"),
                    head_fixed_r = jqobjs.tb_fixed_r.find(".raytable-head"),
                    head = jqobjs.tb_head,
                ...
                if (trc > 0) {
                    ...
                    //修改amend的高度
                    jqobjs.tb_box.find('div.raytable-fixed-amend').height(trh - 1);//1自己下边框
                }
            }//end if
            ...
            if ($fixed_r.children().length > 0) {
                if (tbody_box.scrollWidth > tbody_box.clientWidth) {
                    ...
                    if (tbody_box.scrollHeight > tbody_box.clientHeight)
                        jqobjs.tb_box.find('div.raytable-fixed-amend').removeClass("hidden");

                } else {
                    $fixed_r.addClass("blowout-right");
                    //右侧固定列隐藏时，amend必须隐藏
                    jqobjs.tb_box.find('div.raytable-fixed-amend').addClass("hidden");
                }
            }
        },
        ...
        initPage: function () {
            ...
            var $pagediv = jqobjs.tb_page_box,
                ...
                raylimits = (function () {
                    return [
                        '<select class="raytable-page-limits">',
                        function () {
                            var strTmp = [];
                        ...
                        '</select>',
                        '&nbsp;&nbsp;',
                        '<span class="raytable-page-count">共 ' + page.count + ' 条</span>',
                        '<ul class="raytable-page-pagination">',
                        '</ul>',
                        '<span>到第&nbsp;</span>',
                        '<input type="text" class="rayui-input raytable-page-input" min="1" value="' + curr + '" />&nbsp;页',
                        '&nbsp;&nbsp;',
                        '<a class="rayui-btn raytable-page-btnok">确定</a>'
                    ].join('');
                })();
        ...
        changePaging: function () {
            ...
            var jqobjs = tableCache.jqobjs[that.tableIndex],
                $ul = jqobjs.tb_page_box.find(".raytable-page-pagination"),
                page = options.page,
            ...
            //修改总条数
            jqobjs.tb_page_box.find(".raytable-page-count").html('共 ' + page.count + ' 条');
        },
        clearData: function () {
            var that = this,
                jqobjs = tableCache.jqobjs[that.tableIndex],
        ...
        addEvents: function () {
            ...
            //拖拽调整宽度
            jqobjs.tb_box.find(".raytable-head").on('mousemove.raytable', "th[class*=raytable-cell-resize]", function (e) {
            ...
            }).on('mouseleave.raytable', "th[class*=raytable-cell-resize]", function () {
                if (dict.resizeStart) return;
                $body.css('cursor', '');
            }).on('mousedown.raytable', "th[class*=raytable-cell-resize]", function (e) {
                if (dict.allowResize) {
                    e.preventDefault();
                    var $othis = $(this);
                    dict.elem = this;
            ...
            //拖拽中
            $doc.on('mousemove.raytable', function (e) {
                if (dict.resizeStart) {
                    e.preventDefault();
                    if (dict.elem) {
            ...
            }).on('mouseup.raytable', function (e) {
                if (dict.resizeStart) {
                    dict = {};
                    $body.css('cursor', '');
                }
            ...
            if (tables.length > 1) {
                tables.on("mouseenter.raytable", "tr", function () {
                    tables.find("tr:eq(" + $(this).index() + ")").addClass("raytable-tr-hover");
                }).on("mouseleave.raytable", "tr", function () {
            ...
            $tbodydiv.on("scroll.raytable", funcScrollTop);
            $tbodydiv.hover(function () {
                ...
                $tbodydiv.off("scroll.raytable", funcScrollTop)
                    .on("scroll.raytable", funcScrollTop);
            });

            //checkbox
            var tmpLength = jqobjs.tb_fixed_l.find(".raytable-head-box").length,
                $obj = tmpLength > 0 ? jqobjs.tb_fixed_l : jqobjs.tb_box;
            $obj.on("click.raytableChk", "input.rayui-chk", function () {
                ...
                //同步数据表格当前行选中
                tmpLength > 0 &&
                    (jqobjs.tb_box.find(".raytable-body").find("tr:eq(" + index + ") input.rayui-chk").prop("checked", value));

                //如果是允许多选，需要判断all是否选中
                if (!options.singleSelect) {
                    var chkbox = jqobjs.tb_box.find(".raytable-head input.rayui-chk"),
                        valTmp = !value ? false :
                            $obj.find(".raytable-body input.rayui-chk").length === $obj.find(".raytable-body input.rayui-chk:checked").length;
            ...
            //page分页select
            $page.on("change.raytable", "select.raytable-page-limits", function () {
            ...
            //page页码
            $page.on("click.raytableli", "ul.raytable-page-pagination a:not('.rayui-disabled')", function () {
            ...
            //input页码输入框
            $page.on("keyup.raytable", ".raytable-page-input", function (e) {
                ...
                //修改宽度
                $(this).width(30 + w);
                if (keyCode === 13) $page.find("a.raytable-page-btnok").click();
            });

            //page输入页码跳转
            $page.on("click.raytablebtn", "a.raytable-page-btnok", function () {
                var $input = $page.find(".raytable-page-input"),
                    input = $input.val(),
            ...
            jqobjs.tb_box.on("click.raytableSort", "th.raytable-sort-th,span.raytable-sort-span>i", function (e) {
                e.stopPropagation();
                if (isresizing) {
                    isresizing = false;
                    return;
            ...
            //如果出现省略，则可查看更多
            jqobjs.tb_box.on("click.raytableShow", 'td.nowrap:not("[islink]")', function () {
        ...
        on: function (event, func) {
            ...
            switch (event) {
                //工具条
                case "tool":
                    jqobjs.tb_box.on("click.raytableTool", ".raytable-body td[data-field^=opts] *[event]", function () {
                ...
                //展开行
                case "expand":
                    jqobjs.tb_box.on("click.raytableExpand", "td>a.raytable-expand", function () {
    ...
    $.fn.rayTable = function (option) {
        scrollGap.width || (TableClass.utils.initDefault());
        if ($(this).length === 0) return "DOM对象不存在";
        var classObj = new TableClass(this, option);
